...
public class ResultActivity extends AppCompatActivity implements View.OnClickListener {
   ...
   public  static int final_Score;
    ...
    private void scoreForQuize() {
         final_Score=0;
        if(Utilities.First_Ans.equals("Helsinki")){
            final_Score=final_Score+1;
        }else {
            final_Score=final_Score;
        }

        if(Utilities.Second_Ans.equals("Cairo")){
            final_Score=final_Score+1;
        }else {
            final_Score=final_Score;
        }

        if(Utilities.Third_Ans.equals("Abuja")){
            final_Score=final_Score+1;
        }else {
            final_Score=final_Score;
        }

        if(Utilities.Fourth_Ans.equals("Beijing")){
            final_Score=final_Score+1;
        }else {
            final_Score=final_Score;
        }

        if(Utilities.Fifth_Ans.equals("Ottawa")){
            final_Score=final_Score+1;
        }else {
            final_Score=final_Score;
        }
    ...
    private void setImage(int score) {
        String text="<html><body><font color=#91A1AB> <small> You got </small> </font> <font color=#826FDB><big> "+ score + "/5" + "</big> </font> </body><html>";
        ...
        if(score==5){
            ...
            status_img.setBackgroundResource(R.drawable.smiley);
        }else {
            status_img.setBackgroundResource(R.drawable.poor);
        }
    }
